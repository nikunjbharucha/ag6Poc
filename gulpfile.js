...
gulp.task('compile', ['compile-ts','watch-less']);

// Compile *.ts into *.js
const tsProject = ts.createProject('tsconfig.json');
gulp.task('compile-ts', function () {
    return gulp.src('./src/**/*.ts,./src/*.ts', {base: '.'})
...
gulp.task('compile-less', function () {
    return gulp.src(['./e2e/src/library.less', './e2e/src/bp.less'])//bp.less uses @imports and causes 'assets/less/bp/**/*.less' to compile
        .pipe(less())
...
gulp.task('watch-less', function () {
    ...
    gulp.watch(['./src/*.html','./src/**/*.html', './src/*.css', './src/**/*.js']).on('change', browserSync.reload);
});
